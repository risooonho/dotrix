...
impl Chunk {
    ...
    */
    ...
    fn select_lod(&mut self, camera_position: Vec3) {
        let distance =
    }
    ...
    fn update(
        ...
        if let Some(mesh_id) = self.mesh {
            ...
            if unused_entities > 0 {
                ...
                world.spawn(
                    Some((Model { mesh, texture, transform, ..Default::default() }, index,))
                );
            }
...
pub fn spawn(
    ...
    for x in -chunks_in_view_distance..chunks_in_view_distance {
        for z in -chunks_in_view_distance..chunks_in_view_distance {
            ...
            chunk.select_lod(camera_position);

            // regenerate chunk if changed
            if chunk.changed {
                chunk.update(chunk_index, &mut unused_entities, &mut assets, &mut world);
            }
    ...
    terrain.chunks.retain(|&index, _| {
        ...
        dx * dx + dz * dz < chunks_in_view_distance_2x
    });
